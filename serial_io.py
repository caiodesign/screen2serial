...
def send_right_click(ser: serial.Serial) -> None:
    """
    Send right click command to serial device.
    
    Note: Requires Arduino code to handle "R" command:
        if (cmd == "R") {
...
def send_key(ser: serial.Serial, key: str) -> None:
    """
    Send a key press command to Arduino.
    
    Args:
        ser: Serial connection
        key: Key name (f1, f2, f3, f4, f5, f6, f10, esc)
    """
    ser.write(f"K{key}\n".encode())
    ser.flush()
